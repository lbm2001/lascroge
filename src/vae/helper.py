...
def GRU(x, h_nei, update_gate_linear, reset_gate_input_linear, reset_gate_neighbor_linear, candidate_hidden_linear):
    hidden_size = x.size()[-1]
    sum_h = h_nei.sum(dim=1)
    z_input = torch.cat([x,sum_h], dim=1)
    z = torch.sigmoid(update_gate_linear(z_input))

    r_1 = reset_gate_input_linear(x).view(-1,1, 16) # TODO: Çhange size here
    r_2 = reset_gate_neighbor_linear(h_nei)
    r = torch.sigmoid(r_1 + r_2)
    
    gated_h = r * h_nei
    sum_gated_h = gated_h.sum(dim=1)
    h_input = torch.cat([x,sum_gated_h], dim=1)
    pre_h = torch.tanh(candidate_hidden_linear(h_input))
