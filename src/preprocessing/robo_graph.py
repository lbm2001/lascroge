...
class RoboGraph(nx.Graph):
    """
    Undirected graph representation of a MuJoCo model's body/joint hierarchy.

    Nodes represent both bodies and joints (namespaced by index and name), each
    carrying 'name' and 'type' attributes. Edges connect every joint to the body
    it belongs to and to its parent body, mirroring the model's kinematic tree.
    """
    ...
    def __init__(self, model_xml_path: str, feature_conf_path: str):
        ...
        self.jnt_namespace = {
            i: f"joint_{i}_{self.model.joint(i).name}" for i in range(self.model.njnt)
        }
        self.body_namespace = {
            i: f"body_{i}_{self.model.body(i).name}" for i in range(self.model.nbody)
        }

        self.feature_builder = FeatureMatrixBuilder(
            model_xml_path=str(xml_path), feature_conf_path=str(feature_conf_path)
        )
        ...
        for body_id, joints in body_joints.items():
            ...
            for joint_id in joints:
                if parent_body_id != 0:
                    self.add_edge(
                        self.jnt_namespace[joint_id],
                        self.body_namespace[parent_body_id],
                    )
                    self.add_edge(
                        self.body_namespace[parent_body_id],
                        self.jnt_namespace[joint_id],
                ...
                self.add_edge(
                    self.jnt_namespace[joint_id], self.body_namespace[body_id]
                )
                self.add_edge(
                    self.body_namespace[body_id], self.jnt_namespace[joint_id]
                )
