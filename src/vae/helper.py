...
def tensorize(attr, conn):  # Convert graph data into neural network input format
    ...
    for idx in range(len(attr)):  # Iterate through each graph in the batch
        mod_tree = ModTree(
            attr[idx], conn[idx]
        )  # Create graph from attributes and connectivity
    ...
    for tree in tree_batch:  # Iterate through each graph
        scope.append(
            (len(node_batch), len(tree.nodes))
        )  # Store start index and count for this graph
        ...
        for node in tree.nodes:  # Iterate through nodes in current graph
            if len(node.neighbors) == 1:  # A node with a single neighbor is a leaf
    ...
    for x in node_batch:  # Loops through all nodes
        ...
        for y in x.neighbors:
            mess_dict[(x.idx, y.idx)] = len(
                messages
            )  # Assign a message id to the directed edge x->y
            messages.append((x, y))
    ...
    ]  # For each node, the ids of its incoming messages
    mess_graph = [
        [] for i in range(len(messages))
    ]  # For each message, the messages it should consider
    fmess = [0] * len(messages)  # Source node index for each message, initialized to 0
    ...
    for x, y in messages[1:]:  # Process each message from node x to node y
        ...
        for z in y.neighbors:  # For each neighbor z of node y
            if z.idx == x.idx:  # Skip the sender node x
                continue
            mid2 = mess_dict[(y.idx, z.idx)]  # Get message ID for y→z message
            mess_graph[mid2].append(
                mid1
            )  # Record that message mid2 (y→z) should consider message mid1 (x→y)

    # Padding the node_graph and mess_graph to ensure they have the same length
    max_len = max(
        [len(t) for t in node_graph] + [1]
    )  # + [1] guards against an empty node_graph
    for t in node_graph:
...
def tree_to_adjacency(tree_root):
    ...
    num_nodes = len(
        nodes
    )  # To force output shape == input shape, drop num_nodes here and pass it in as a parameter instead
