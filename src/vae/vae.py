...
class Encoder(nn.Module):
    ...
    def rsample(self, z_vecs):  # W_mean and W_var are nn.Linear layers
        ...
        z_mean = self.W_mean(
            z_vecs
        )  # Apply linear Layer to compute mean of latent Gaussian distribution, z_mean shape is [batch_size, LATENT_SIZE]
        ...
        kl_loss = (
            ...
            * torch.sum(1.0 + z_log_var - z_mean * z_mean - torch.exp(z_log_var))
            / batch_size
        )  # Dividing by batch_size gives average over batch
        epsilon = create_var_float(torch.randn_like(z_mean))  # epsilon ~ N(0,I)
        z_vecs = (
            z_mean + torch.exp(z_log_var / 2) * epsilon
        )  # z_vecs = \mu + \sigma * \epsilon
    ...
    def encode(self, jtenc_holder):
        ...
        # Pool each tree's leaf node vectors into a single vector per tree.
        # leafs is a list of lists; each inner list holds the node indices of one tree's leaves.
        batch_vecs = []
        for leaf in leafs:
            cur_vecs = torch.zeros_like(
                node_vecs[0]
            )  # node_vecs has shape [n_nodes, hidden_size]
            for node_idx in leaf:
                cur_vecs += node_vecs[
                    node_idx
                ]  # Sum the vectors of all leaf nodes in this tree
            if self.encoding_method == "average":
                cur_vecs /= len(leaf)
            elif self.encoding_method != "sum":
                exit("Encoding method is not in the list")
            batch_vecs.append(cur_vecs)

        tree_vecs = torch.stack(batch_vecs, dim=0)
        return tree_vecs, messages  # tree_vecs is the encoder output
...
class Decoder(nn.Module):
    def __init__(self, hidden_size, max_nb, latent_size, feature_dim):
        ...
        self.W_o_categorical = nn.Linear(
            self.hidden_size, 1
        )  # For body/joint classification (logits)
        self.W_o_continuous = nn.Linear(
            self.hidden_size, self.feature_dim - 1
        )  # For the remaining continuous features
        self.U_o = nn.Linear(self.hidden_size, 1)  # Output layer for stop prediction

        self.features_to_dim = nn.Linear(
            self.feature_dim, self.hidden_size
        )  # Linear layer to project features to hidden size
    ...
    def forward(self, mol_batch, x_tree_vecs):
        ...
        pred_hiddens.append(
            create_var_int(torch.zeros(batch_size, self.hidden_size))
        )  # Initial hidden states are zeros (no previous context)
        pred_targets.extend(
            [mol_tree.nodes[0].features for mol_tree in mol_batch]
        )  # Actual features of root nodes
        pred_contexts.append(
            create_var_int(torch.LongTensor(range(batch_size)))
        )  # Maps each prediction to its corresponding tree in the batch

        max_iter = max(
            [len(tr) for tr in traces]
        )  # Maximum number of steps needed (longest trace in batch)
        padding = create_var_int(
            torch.zeros(self.hidden_size), False
        )  # Zero vector used for padding when nodes have fewer neighbors
        ...
        for t in range(max_iter):
            prop_list = []  # Flattened traces (s_11, s_12, s_21, s_22); e.g. 11 = first node of first tree
            ...
            for i, plist in enumerate(traces):
                if t < len(plist):
                    ...
                    batch_list.append(
                        i
                    )  # Keep track of which tree each active node belongs to
            ...
            for node_x, real_y, _ in prop_list:
                # Neighbors for message passing (the target node real_y we move to is excluded)
                cur_nei = [
                    h[(node_y.idx, node_x.idx)]
                    for node_y in node_x.neighbors
                    if node_y.idx != real_y.idx
                ]  # For each active node, collect neighbor hidden states
                pad_len = (
                    self.max_nb - len(cur_nei)
                )  # Pad to fixed size self.max_nb (maximum neighbors) for batch processing
                ...
                ]  # Stop prediction: Include ALL neighbors (different from message passing)
                ...
                cur_x.append(
                    node_x.features
                )  # Node features: Collect current node's features

            # Clique embedding
            cur_x = create_var_float(
                torch.FloatTensor(np.array(cur_x, dtype=np.float32))
            )
            cur_x = self.features_to_dim(cur_x)  # Skip embedding for now

            # Message passing
            cur_h_nei = torch.stack(cur_h_nei, dim=0).view(
                -1, self.max_nb, self.hidden_size
            )
            new_h = GRU(
                cur_x, cur_h_nei, self.W_z, self.W_r, self.U_r, self.W_h
            )  # GRU message passing: Update hidden states using GRU with neighbor information

            # Node Aggregate
            cur_o_nei = torch.stack(cur_o_nei, dim=0).view(
                -1, self.max_nb, self.hidden_size
            )
            cur_o = cur_o_nei.sum(
                dim=1
            )  # Aggregation: Sum neighbor hidden states for stop prediction
            ...
            for i, m in enumerate(prop_list):
                ...
                h[(x, y)] = new_h[
                    i
                ]  # Hidden state storage: Store computed hidden states for later use
                node_y.neighbors.append(
                    node_x
                )  # Neighbor building: Incrementally build neighbor connections
                if direction == 1:  # If this is a forward step in DFS
                    pred_target.append(
                        node_y.features
                    )  # Target Features: Collect features that need to be predicted
                    pred_list.append(i)  # Which nodes need prediction
                stop_target.append(
                    direction
                )  # Stop targets: Collect stop decisions (1=continue, 0=stop)
            ...
            stop_hidden = torch.cat(
                [cur_x, cur_o], dim=1
            )  # cur_x current node features, cur_o aggregated neighbor information
            stop_hiddens.append(stop_hidden)
            stop_contexts.append(cur_batch)
            stop_targets.extend(
                stop_target
            )  # Stop data: Store hidden states, contexts, and targets for stop prediction
        ...
        stop_hidden = torch.cat(
            [cur_x.unsqueeze(0) if cur_x.dim() == 0 else cur_x, cur_o], dim=1
        )
        stop_hiddens.append(stop_hidden)
        ...
        pred_scores_categorical = self.aggregate(
            pred_hiddens, pred_contexts, x_tree_vecs, "features_categorical"
        )  # Feature prediction: Use aggregate function to predict node features
        pred_scores_continuous = self.aggregate(
            pred_hiddens, pred_contexts, x_tree_vecs, "features_continuous"
        )

        # Convert pred_targets to tensor for regression
        pred_targets = torch.tensor(
            np.array(pred_targets, dtype=np.float32), dtype=torch.float32
        )
        pred_targets_tensor = create_var_int(
            torch.FloatTensor(np.array(pred_targets))
        )  # was LongTensor before
        ...
        categorical_loss = F.binary_cross_entropy_with_logits(
            pred_scores_categorical, pred_targets_categorical, reduction="sum"
        ) / len(mol_batch)  # Categorical loss for feature prediction
        continuous_loss = self.pred_loss_nn(
            pred_scores_continuous, pred_targets_continuous
        ) / len(mol_batch)  # Regression loss for feature prediction
        pred_loss = (
            categorical_loss + continuous_loss
        )  # Combine losses for feature prediction
        ...
        )  # Stop prediction: Use aggregate function to predict stop decisions
        ...
        stops = torch.ge(
            stop_scores, 0
        ).float()  # checks if each score is ≥ 0; If score ≥ 0 → decision is 1 (continue expanding) , If score < 0 → decision is 0 (stop expanding)
        stop_acc = torch.eq(
            stops, stop_targets
        ).float()  # compares each prediction with the correct answer; .float() converts to 1.0 (correct) or 0.0 (incorrect)
        stop_acc = (
            torch.sum(stop_acc) / stop_targets.nelement()
        )  # The percentage of nodes where the model correctly predicted whether to stop or continue expanding that branch.
    ...
    def decode(self, x_tree_vecs, prob_decode, max_decode_len):
        ...
        # Root Prediction
        root_categorical = self.aggregate(
            init_hiddens, contexts, x_tree_vecs, "features_categorical"
        )
        root_continuous = self.aggregate(
            init_hiddens, contexts, x_tree_vecs, "features_continuous"
        )
        # We don't apply sigmoid inside the layer because training uses BCEWithLogitsLoss (more stable),
        # so sigmoid must be applied explicitly here at decode time.
        root_categorical_binary = (torch.sigmoid(root_categorical) > 0.5).float()
        root_features = torch.cat(
            [root_categorical_binary.view(-1), root_continuous.view(-1)], dim=-1
        )
        ...
        for step in range(max_decode_len):
            ...
            cur_x = create_var_float(
                node_x.features.detach().clone().float().unsqueeze(0)
            )
            ...
            if not backtrack:  # Forward: Predict next clique
                ...
                pred_categorical = self.aggregate(
                    new_h, contexts, x_tree_vecs, "features_categorical"
                )
                pred_continuous = self.aggregate(
                    new_h, contexts, x_tree_vecs, "features_continuous"
                )
                pred_categorical_binary = (
                    torch.sigmoid(pred_categorical) > 0.5
                ).float()
                predicted_features = (
                    torch.cat(
                        [pred_categorical_binary.view(-1), pred_continuous.view(-1)],
                        dim=-1,
                    )
                    .squeeze()
                    .detach()
...
class VAE(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder(
            self.hidden_size, self.max_nb, self.latent_size, self.feature_dim
        )
    ...
    def decode(self, z_tree_vecs, prob_decode):
        ...
        root, all_nodes = self.decoder.decode(
            z_tree_vecs, prob_decode=prob_decode, max_decode_len=self.max_decode_len
        )
